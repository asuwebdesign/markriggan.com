module.exports = function(grunt) {
	...
	grunt.initConfig({
		 ...
		 * https://www.npmjs.org/package/grunt-fontello
		 * Download font library from fontello.com
		 */
		fontello: {
			dist: {
				options: {
					config  : '<%= project.fonts %>/config.json',
					fonts   : '<%= project.fonts %>/',
					styles  : '<%= project.sass %>/vendors/',
					sass    : true,
					force   : true
				}
			}
	 ...
	 */
	grunt.registerTask('default', ['bower', 'uglify', 'fontello', 'compass', 'autoprefixer', 'cssmin']);
